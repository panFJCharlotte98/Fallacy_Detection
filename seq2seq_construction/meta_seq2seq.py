...
class MultiTaskWrapper(Dataset):
    def __init__(self, args_path2dataset, meta_args, section):
        ...
        # args_path2dataset is like:
        # {"META_TUNING/wikiqa.cfg":Dataset classes in seq2seq_construction/task_name.py, "task.cfg": Dataset, ...}
        # Raw data and size.
        args_path2data = {}
        for args_path, dataset in args_path2dataset.items():
            # dataset: TrainDataset class in seq2seq_construction/task_name.py
            # dataset[idx] = {"id":, "question":, "table_id": ,"table":, "passage":, ..., "struct_in": ,"text_in": ,"seq_out": }
            # which is one dictionary item of datset.extended_data
...
class Constructor(object):
    def __init__(self, meta_args):
        # meta_args: the Args() object --- cofig setting of an experiment (config/experiment/xxx.cfg)
        # args in train.py
    ...
    def to_seq2seq(self, raw_datasets_dict: Dict[str, DatasetDict]):
        ...
        @return:
        """
        train_dev_test_data_of_tasks = {'train': {}, "dev": {}, "test": {}}
        for arg_path, dataset in raw_datasets_dict.items():
            # arg_path: task/task_name.cfg
            # dataset: Dataset class in seq2seq_construction/task_name.py
            if len(dataset) == 2:
                train_dev_test_data_of_tasks['train'][arg_path] = dataset[0]
                train_dev_test_data_of_tasks['dev'][arg_path] = dataset[1]
        ...
        dev_dataset = DevDataset(self.meta_args, train_dev_test_data_of_tasks['dev'])
...
class TrainDataset(Dataset):
    """
    Using the meta-tuning policy to control the data feeding order.
    """
